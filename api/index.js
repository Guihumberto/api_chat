...
app.get("/app/instructions", (req, res) => {
    const userAgent = req.get('User-Agent') || '';
    const isInstagram = userAgent.includes('Instagram') || 
                      userAgent.includes('FBAN') || 
                      userAgent.includes('FBAV');
    ...
    if (!isInstagram) {   
      return res.send(criarPaginaInstrucoes());
    } else {
      // return res.send(getInstagramRedirectHTML(fullUrl));
      return res.redirect(fullUrl);
    }
...
function criarPaginaInstrucoes() {
  return `<!DOCTYPE html>
<html lang="pt-BR">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Como Abrir no Navegador</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="container">
        ...
        <div class="header">
            <div class="title">📱 Como Abrir no Navegador</div>
            <div class="subtitle">Siga os passos abaixo para uma melhor experiência</div>
        </div>
        
        <div style="margin-top: 20px;">
            <div class="instruction-text">
                👆 <span class="highlight">Clique nos 3 pontos</span> no canto superior direito
            </div></div>
            
            <div class="steps">
                <div class="step">
                    <span class="step-number">1.</span> Toque nos <span class="highlight">três pontos (⋮)</span> no canto superior direito
                </div>
                <div class="step">
                    <span class="step-number">2.</span> Selecione <span class="highlight">"Abrir no navegador externo"</span>
                </div>
                <div class="step">
                    <span class="step-number">3.</span> Aproveite a <span class="highlight">experiência completa</span> do nosso site!
                </div>
            </div>
        </div>

        <a href="https://app.estudodalei.com.br/app" class="back-button">
          Voltar
        </a>
    </div>
    <script src="/utils/instagram.js"></script>
